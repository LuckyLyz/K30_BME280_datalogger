...
// RToledo-Crow NGENS-ASRC-CUNY, Wiley H NIST, 7.15.18
...
#define SEALEVELPRESSURE_HPA (1013.25) // for the pressure reading
...
void setup(void) {
  ...
  if (! bme.begin()) {
    error("Couldn't find BME280 sensor");
  }
...
void loop(void)  {
  ...
  logfile.print(" Temp = ");
  logfile.print(bme.readTemperature(), DEC);
  logfile.print(" *C, ");
  logfile.print(" Pres = ");
  logfile.print(bme.readPressure() / 100.0F, DEC);
  logfile.print(" hPa, ");
  logfile.print(" Approx. Altitude = ");
  logfile.print(bme.readAltitude(SEALEVELPRESSURE_HPA), DEC);
  logfile.print(" m, ");
  logfile.print(" RH = ");
  logfile.print(bme.readHumidity(), DEC);
  logfile.print(" %, ");
  logfile.print("  CO2 = ");
  ...
  Serial.print(" Temp = ");
  Serial.print(bme.readTemperature(), DEC);
  Serial.print(" *C, ");
  Serial.print(" Pres = ");
  Serial.print(bme.readPressure() / 100.0F, DEC);
  Serial.print(" hPa, ");
  Serial.print(" Approx. Altitude = ");
  Serial.print(bme.readAltitude(SEALEVELPRESSURE_HPA), DEC);
  Serial.print(" m, ");
  Serial.print(" RH = ");
  Serial.print(bme.readHumidity(), DEC);
  Serial.print(" %, ");
  Serial.print("  CO2 = ");
